        ...
        public PartialViewResult _SwitchRecords()
        {
            ...
            using (var conn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString))
            {
                try
                {
                    conn.Open();
                    string query = "";
                    query += "select TubeWellId, ResourceLocation, [Remote], PumpStatus, InsertionDateTime ";
                    query += "from(select t.TubeWellId, r.ResourceLocation, t.[Remote], t.[PumpStatus], t.[InsertionDateTime], ";
                    query += "ROW_NUMBER() OVER(PARTITION BY t.TubeWellId ORDER BY t.InsertionDateTime DESC) AS ROWNO ";
                    query += " from tblTubewellsTest t ";
                    query += " inner join tblResource r on t.TubeWellId = r.ResourceID ";
                    query += " ) AS tempTable ";
                    query += " WHERE ROWNO = 1 ";
                    query += " order by TubeWellId";
