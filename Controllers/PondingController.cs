...
namespace WASA_EMS.Controllers
{
    public class PondingController : Controller
    {
        ...
        public List<PondingTableClass> getPondingTableList()
        {
            DataTable dtRes = new DataTable();
            DataTable Dashdt = new DataTable();
            var spellDataList = new List<SpellData>();
            ...
            using (SqlConnection conn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["DefaultConnectionRMS"].ConnectionString))
            {
                try
                {
                    ...
                    foreach (DataRow drRes in dtRes.Rows)
                    {
                        ...
                        if (Dashdt.Rows.Count > 0)
                        {
                            PondingTableClass sd = getAllSpells(Dashdt, Convert.ToInt32(drRes["resourceNumber"]));
                            tablList.Add(sd);
                        }
                    }
                }
        ...
        /// <summary>
        /// //////////////////////////////////////////////
        public PondingTableClass getAllSpells(DataTable dt, int order)
        {
                ...
                try
                {
                    ...
                    using (SqlDataReader sdr = cmd.ExecuteReader())
                    {
                        while (sdr.Read())
                        {
                            ...
                            if (Convert.ToDouble(sdr["ponding"]) == maxVal)
                            {
